...
coolApp.service('api', function ($http) {
    ...
    var base = 'https://cooltasksapp.azurewebsites.net';
    ...
    let httpCall = function (method, jsonData, next, path) {
        ...
        $http(settings).success(function (response) {
            ...
            if (response.ok) {
                ...
                console.error("error with http api")
            }
        });
    };
    ...
    this.updateOne = function updateOne(data, next) {
        httpCall('PUT', data, next, base + '/api/tasks/edit');
    }

    this.completeOne = function completeOne(data, next) {
